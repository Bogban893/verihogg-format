...
class FormatArgsTest : public ::testing::Test {
 protected:
  ...
  // Добавляет "formatter" в начало и вызывает parseCommandLine.
  ...
  // Хелпер: парсит аргументы и возвращает стиль.
  ...
  // Хелпер для тестов, ожидающих исключение из buildStyle.
  // Вызывать после parse() вручную.
...
// ---------------------------------------------------------------------------
// Числовые флаги — каждый флаг в изоляции,
...
TEST_F(FormatArgsTest, WithoutInplaceFlagRunConfigIsFalse) {
  auto [style, run] = buildStyle({"--column_limit", "80"});
...
TEST_F(FormatArgsTest, MaxUint32ColumnLimit) {
  // uint32_t max = 4294967295.
  auto [style, run] = buildStyle({"--column_limit", "4294967295"});
  EXPECT_EQ(style.column_limit, 4294967295U);
}
